...
namespace everest
{
	namespace hwdrivers
	{
	    struct TLidarScan
	    {
            ...
            //lidar of the linux time stamp
            struct timespec lidar_time;

            void setLidarTime(struct timespec &time)
            { 
                lidar_time.tv_sec = time.tv_sec;
                lidar_time.tv_nsec = time.tv_nsec;
            }

            struct timespec getLidarTime() { return lidar_time;}
	    };
		...
		class C3iroboticsLidar
		{
            public:
                ...
                float getLidarOffsetAngle() {return m_lidar_offset_angle;}
